    ...
    methods: {
        validateForm: function() {
          ...
          validateFullName: function() {
            this.formErrors.full_name = this.newPatient.full_name ? '' : 'Full Name is required.';
            if (this.newPatient.full_name && !/^[a-zA-Z\s]+$/.test(this.newPatient.full_name)) {
              this.formErrors.full_name = 'Invalid characters in Full Name.';
            }
          },
          
          validateBirthdayDate: function() {
            this.formErrors.birthday_date = this.newPatient.birthday_date ? '' : 'Birthday Date is required.';
            if (this.newPatient.birthday_date && !/^\d{4}-\d{2}-\d{2}$/.test(this.newPatient.birthday_date)) {
              this.formErrors.birthday_date = 'Invalid Birthday Date format. Use YYYY-MM-DD.';
            }
          },
      
          validatePhoneNumber: function() {
            this.formErrors.phone_number = this.newPatient.phone_number ? '' : 'Phone Number is required.';
            if (this.newPatient.phone_number && !/^[0-9]+$/.test(this.newPatient.phone_number)) {
              this.formErrors.phone_number = 'Phone Number should contain only digits.';
            }
          },
      
          validateGender: function() {
            this.formErrors.gender = this.newPatient.gender ? '' : 'Gender is required.';
          },
